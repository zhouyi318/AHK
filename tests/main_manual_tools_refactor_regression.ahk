...
try {
    ...
    if !InStr(source, '"onBotSimChanged", ObjBindMethod(mainUiActionBridgeService, "InitWindowState")')
        throw Error("Main should bind onBotSimChanged through the UI action bridge instead of a fragile raw callback")
    if InStr(source, '"onBotSimChanged", "InitWindowState"')
        throw Error("Main should not pass onBotSimChanged to ManualTools as a raw callback string")
    if InStr(source, '"onBotSimChanged", Func("InitWindowState")')
        throw Error("Main should not bind onBotSimChanged through Func() in the startup path")
